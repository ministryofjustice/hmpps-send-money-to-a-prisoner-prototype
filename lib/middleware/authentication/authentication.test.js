  ...
  describe('when it runs in production', () => {
    ...
    describe('server with no username/password set', () => {
      ...
      it('should send marked up error message', () => {
        let errorDisplayedto user = res.send.mock.calls[0][0]
        expect(errorDisplayedto user).toBe('<h1>Error:</h1><p>Username or password not set. <a href="https://govuk-prototype-kit.herokuapp.com/docs/publishing-on-heroku#6-set-a-username-and-password">See guidance for setting these</a>.</p>')
      })
    })
